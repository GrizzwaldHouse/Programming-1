...
public: // <----- Your enumerator must be below this line
	...
	*/
    ...
    {
		MathOperator x = MathOperator::Add;
		...
		if (input == 0) {
			...
			x = MathOperator::Subtract;
		}
		else if (input == 2) {
			x = MathOperator::Multiply;
		}
		else if (input == 3) {
			x = MathOperator::Divide;
		}
		else if (input == 4) {
			x = MathOperator::Modulo;
		}
	...
	*/
    ...
    {// Initialize a placeholder result
		int result = 0;
		//Set Operatoration to Sum adding two inputs
		if (operation == Submission::MathOperator::Add) {
    ...
    */
    static double Test4(double input, bool kiloToLb)
	{
		...
		result = (kiloToLb) ? input * 2.2 : input * 0.45;
